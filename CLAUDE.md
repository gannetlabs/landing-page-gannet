...
**Hero** — `components/sections/Hero.tsx` — layout de 2 columnas en desktop (`lg:grid-cols-2`). Izquierda: H1 con `leading-[0.96]` y `clamp(2.75rem, 5vw, 4.5rem)`, énfasis verde en `"resuelve"`. Descripción, CTAs, stats de social proof (`border-t border-white/[0.12]`, labels `text-white/55`). Derecha: `GannetBirdAnimation size={400}` — whitemark estático, sin hover. Sin `birdHovered` state. 5 anillos concéntricos (`380px`–`1500px`) en capa `z-[1]`, centrados en `left: 75%` del `max-w-7xl` container. 17 floating tech tags en capa `z-[2]` full-section-width, animadas con CSS puro (`tag-float` + `tag-glow` keyframes en `globals.css`) — **sin Framer Motion en las tags**. Cada tag tiene `floatDur/floatDelay/glowDur/glowDelay` independientes → glow auto-illuminate escalonado. Tags se ubican en zona derecha (cerca del ave), centro y borde izquierdo. Contenido en `z-10` (sobre anillos y tags). Mobile: columna derecha oculta. `prefers-reduced-motion`: `.hero-tag { animation: none }` en globals.css. **Interacción cursor-rings**: `onMouseMove` en el `<section>` trackea posición del cursor; `sectionRef` en el section y `containerRef` en el `max-w-7xl` interior permiten calcular la distancia al centro de los anillos (`left: 75%` del container). El anillo más cercano al cursor recibe `className="ring-active"` + opacidad 4.5× + `box-shadow` verde de 3 capas + `transition` suave. Un div de glow (320px, radial-gradient verde) sigue al cursor dentro de la capa `z-[1]`. Keyframe `ring-pulse` y `.ring-active` definidos en `globals.css`; `prefers-reduced-motion` desactiva el pulso pero mantiene el highlight. Sin interacción en mobile (anillos ocultos en breakpoints < lg).
